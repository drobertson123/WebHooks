...
{
    /// <summary>
    /// An <see cref="IAsyncResourceFilter"/> that verifies the AWS signature header. Confirms the header exists,
    ...
    public class VerifySignatureFilter : WebHookVerifySignatureFilter, IAsyncResourceFilter
    {
        ...
        /// <summary>
        /// Instantiates a new <see cref="AWSVerifySignatureFilter"/> instance.
        /// </summary>
        ...
        /// </param>
        public AWSVerifySignatureFilter(
        ...
        /// <inheritdoc />
        public override string ReceiverName => AWSConstants.ReceiverName;
        ...
        public async Task OnResourceExecutionAsync(ResourceExecutingContext context, ResourceExecutionDelegate next)
        {
            ...
            if (HttpMethods.IsPost(request.Method))
            {
                ...
                var header = GetRequestHeader(request, AWSConstants.SignatureHeaderName, out errorResult);
                if (errorResult != null)
                {
                    context.Result = errorResult;
                    return;
                ...
                if (values.Count != 2 ||
                    !StringSegment.Equals(
                        headerKey,
                        AWSConstants.SignatureHeaderKey,
                        StringComparison.OrdinalIgnoreCase))
                {
                    Logger.LogWarning(
                        0,
                        $"Invalid '{AWSConstants.SignatureHeaderName}' header value. Expecting a value of " +
                        $"'{AWSConstants.SignatureHeaderKey}=<value>'.");

                    var message = string.Format(
                        CultureInfo.CurrentCulture,
                        Resources.SignatureFilter_BadHeaderValue,
                        AWSConstants.SignatureHeaderName,
                        AWSConstants.SignatureHeaderKey,
                        "<value>");
                ...
                var expectedHash = FromHex(headerValue, AWSConstants.SignatureHeaderName);
                if (expectedHash == null)
                {
                    context.Result = CreateBadHexEncodingResult(AWSConstants.SignatureHeaderKey);
                    return;
                }

                // 3. Get the configured secret key.
                var secretKey = GetSecretKey(ReceiverName, context.RouteData, AWSConstants.SecretKeyMinLength);
                if (secretKey == null)
                {
                ...
                if (!SecretEqual(expectedHash, actualHash))
                {
                    ...
                    errorResult = CreateBadSignatureResult(AWSConstants.SignatureHeaderName);
