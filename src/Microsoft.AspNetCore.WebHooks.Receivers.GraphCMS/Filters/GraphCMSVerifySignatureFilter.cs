...
{
    /// <summary>
    /// An <see cref="IAsyncResourceFilter"/> that verifies the GraphCMS signature header. Confirms the header exists,
    /// reads Body bytes, and compares the hashes.
    /// </summary>
    public class VerifySignatureFilter : WebHookVerifySignatureFilter, IAsyncResourceFilter
    {
        // Character that appears between the key and value in the signature header.
        private static readonly char[] PairSeparators = new[] { '=' };

        /// <summary>
        /// Instantiates a new <see cref="GraphCMSVerifySignatureFilter"/> instance.
        /// </summary>
        ...
        /// </param>
        public GraphCMSVerifySignatureFilter(
        ...
        /// <inheritdoc />
        public override string ReceiverName => GraphCMSConstants.ReceiverName;
        ...
        public async Task OnResourceExecutionAsync(ResourceExecutingContext context, ResourceExecutionDelegate next)
        {
            ...
            if (HttpMethods.IsPost(request.Method))
            {
                ...
                var header = GetRequestHeader(request, GraphCMSConstants.SignatureHeaderName, out errorResult);
                if (errorResult != null)
                {
                    context.Result = errorResult;
                    return;
                ...
                if (values.Count != 2 ||
                    !StringSegment.Equals(
                        headerKey,
                        GraphCMSConstants.SignatureHeaderKey,
                        StringComparison.OrdinalIgnoreCase))
                {
                    Logger.LogWarning(
                        0,
                        $"Invalid '{GraphCMSConstants.SignatureHeaderName}' header value. Expecting a value of " +
                        $"'{GraphCMSConstants.SignatureHeaderKey}=<value>'.");

                    var message = string.Format(
                        CultureInfo.CurrentCulture,
                        Resources.SignatureFilter_BadHeaderValue,
                        GraphCMSConstants.SignatureHeaderName,
                        GraphCMSConstants.SignatureHeaderKey,
                        "<value>");
                ...
                var expectedHash = FromHex(headerValue, GraphCMSConstants.SignatureHeaderName);
                if (expectedHash == null)
                {
                    context.Result = CreateBadHexEncodingResult(GraphCMSConstants.SignatureHeaderKey);
                    return;
                }

                // 3. Get the configured secret key.
                var secretKey = GetSecretKey(ReceiverName, context.RouteData, GraphCMSConstants.SecretKeyMinLength);
                if (secretKey == null)
                {
                ...
                if (!SecretEqual(expectedHash, actualHash))
                {
                    ...
                    errorResult = CreateBadSignatureResult(GraphCMSConstants.SignatureHeaderName);
